...
def clean_ccg_data(ccg_concat):
    ...
    ccg_concat = ccg_concat.drop_duplicates(
        subset=['ccg_code', 'ccg_name', 'stp_code', 'stp_name'])
    return ccg_concat
...
def merge_stp_data(stp, ccg_concat):
    end_lookup = ccg_concat.merge(
        stp, left_on='stp_code', right_on='STP20CD', how='left')

    end_lookup.stp_name = end_lookup.stp_name.replace(
        'Cornwall and the Isles of Scilly Health and Social Care Partnership', 'Cornwall and the Isles of Scilly')
    end_lookup.stp_name = end_lookup.stp_name.replace(
        'Frimley Health and Care ICS', 'Frimley Health')
    end_lookup.stp_name = end_lookup.stp_name.replace(
        'Surrey Heartlands Health and Care Partnership', 'Surrey Heartlands')
    end_lookup.stp_name = end_lookup.stp_name.replace(
        'Sussex and East Surrey Health and Care Partnership', 'Sussex Health and Care Partnership')
    end_lookup.stp_name = end_lookup.stp_name.replace(
        'Sussex and East Surrey', 'Sussex Health and Care Partnership')
...
def export_result_data(end_lookup):
    end_lookup.to_csv('server/app/static_data/combined_lookup_download.csv')
